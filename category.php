...
<body>
                ...
                <div class="row">
                    <ul class="gategory-product">
                        ...
                        <?php 
                            ...
                            // Xác định trang hiện tại và số sản phẩm mỗi trang
                            $productsPerPage = 8;
                            ...
                            // Truy vấn SQL lấy sản phẩm theo loại
                            $sql = "SELECT * FROM san_pham WHERE LSP_MA = '$maloaisp' LIMIT $offset, $productsPerPage";
                            $result = $conn->query($sql);

                            // Truy vấn để đếm tổng số sản phẩm
                            $totalProductsResult = $conn->query("SELECT COUNT(*) as total FROM san_pham WHERE LSP_MA = '$maloaisp'");
    ...
    <div class="pagination" style="margin-top: 20px;">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="?maloaisp=<?php echo $maloaisp; ?>&page=<?php echo $i; ?>" 
            style="margin: 0 5px; padding: 8px 15px; background: <?php echo ($i == $page) ? '#c43740' : '#f1f1f1'; ?>; 
